...
export default function CompletedTasks({ includedTasks, currentUser, onTaskCompleted, onTaskDeleted }: AssignTaskProps) {
  ...
  return (
    <div>
      ...
      {userTasks?.length === 0 ? (
        <p className="text-gray-500 mb-6">No completed tasks assigned to you.</p>
      ) : (
      ...
      <h3 className="text-xl font-semibold mb-4">Other Completed Tasks</h3>
      {otherTasks?.length === 0 ? (
        <p className="text-gray-500">No other completed tasks in this trip.</p>
      ) : (
        <ul className="mb-6 space-y-3">
          {otherTasks?.map(task => (
            <TaskCard 
